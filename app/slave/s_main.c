...
* CC2500 WOR off; both radios SPWD; MCU Stop 3 (~1 µA).
* RTC period lengthens with ship time to limit battery use on long storage;
* auto-exit to NORMAL after SLAVE_SHIP_MAX_ELAPSED_SEC (default 365 days).
* WKUP pin still forces immediate exit to NORMAL.
*
* Schedule (g_ship_elapsed_sec since CMD_SHIP, updated each RTC wake):
*   [0, 2h)     → wake every 5 min
*   [2h, 3h)   → every 30 min
*   [3h, 30d)  → every 1 h
*   [30d, 60d) → every 2 h
*   [60d, 365d)→ every 3 h
*   ≥ 365d     → auto-exit NORMAL (radios + Stop 2 beacon path restored) */
#define SLAVE_SHIP_SEC_5MIN           300U
#define SLAVE_SHIP_SEC_30MIN          1800U
#define SLAVE_SHIP_SEC_1H             3600U
#define SLAVE_SHIP_SEC_2H             7200U
#define SLAVE_SHIP_SEC_3H             10800U
/* End of “5 min interval” phase; default 2 h (then 30 min until SLAVE_SHIP_PHASE_30MIN_END_SEC). */
#ifndef SLAVE_SHIP_PHASE_5MIN_END_SEC
#define SLAVE_SHIP_PHASE_5MIN_END_SEC   (2U * 3600U)
#endif
#define SLAVE_SHIP_PHASE_30MIN_END_SEC  (3U * 3600U)       /* switch to 1 h at 3 h */
#define SLAVE_SHIP_PHASE_1H_END_SEC     (30U * 86400U)     /* switch to 2 h at 30 d */
#define SLAVE_SHIP_PHASE_2H_END_SEC     (60U * 86400U)     /* switch to 3 h at 60 d */
#define SLAVE_SHIP_MAX_ELAPSED_SEC      (365U * 86400U)   /* auto-exit at 365 d */
...
typedef struct {
   ...
   /* ship mode counters */
   uint32_t ship_entries;          /* times Stop3 ship sleep was entered */
   uint32_t ship_rtc_wakeup_count; /* RTC wakes while in ship mode */
} Slave_PowerTrace;
...
/* Cumulative ship time (sum of RTC sleep intervals); approximates wall time in ship. */
static uint32_t g_ship_elapsed_sec       = 0U;
static uint32_t g_ship_last_interval_sec = 0U; /* last LP_ConfigureRTCWakeUp(seconds) */
...
/* Next RTC period from cumulative ship time (see file-header schedule). */
static uint32_t Slave_ShipRtcPeriodForElapsed(uint32_t elapsed_sec)
{
   if (elapsed_sec < SLAVE_SHIP_PHASE_5MIN_END_SEC) {
       return SLAVE_SHIP_SEC_5MIN;
   }
   if (elapsed_sec < SLAVE_SHIP_PHASE_30MIN_END_SEC) {
       return SLAVE_SHIP_SEC_30MIN;
   }
   if (elapsed_sec < SLAVE_SHIP_PHASE_1H_END_SEC) {
       return SLAVE_SHIP_SEC_1H;
   }
   if (elapsed_sec < SLAVE_SHIP_PHASE_2H_END_SEC) {
       return SLAVE_SHIP_SEC_2H;
   }
   if (elapsed_sec < SLAVE_SHIP_MAX_ELAPSED_SEC) {
       return SLAVE_SHIP_SEC_3H;
   }
   return SLAVE_SHIP_SEC_3H;
}
...
static void Slave_EnterShipSleep(void)
{
   ...
   /* Already exceeded max ship time — force NORMAL without another Stop 3. */
   if (g_ship_elapsed_sec >= SLAVE_SHIP_MAX_ELAPSED_SEC) {
       g_power_profile          = SLAVE_RUN_NORMAL;
       g_ship_elapsed_sec       = 0U;
       g_ship_last_interval_sec = 0U;
   ...
   g_ship_last_interval_sec = Slave_ShipRtcPeriodForElapsed(g_ship_elapsed_sec);
   LP_ConfigureRTCWakeUp(&g_lp, g_ship_last_interval_sec);
   ...
   if (src & LP_WAKEUP_RTC_ALARM) {
       g_ship_elapsed_sec += g_ship_last_interval_sec;
       g_slave_pwr_trace.ship_rtc_wakeup_count++;
       g_slave_pwr_trace.wakeup_rtc_count++;

       g_sensor_data[8] = (uint8_t)(g_ship_elapsed_sec & 0xFFU);
   ...
   while (session_active) {
       if (g_stream_enabled) {
           ...
           while (g_stream_enabled && session_active) {
               ...
               /* >= : discrete steps land on multiples of g_stream_interval_ms;
                * > would run one extra stream period after hitting the limit. */
